...
export function useAnalyticsOverview(period: DashboardPeriod = '30d', enabled = true) {
  const { user } = useAuth();

  return useQuery({
    queryKey: queryKeys.analytics.overview(period),
    queryFn: () => fetchAnalyticsOverview(period),
    enabled: Boolean(user?.onboarding_completed) && enabled,
  });
}

export function useAnalyticsEngagement(period: DashboardPeriod = '30d', enabled = true) {
  const { user } = useAuth();
...
export function useAnalyticsDomains(period: DashboardPeriod = '30d', enabled = true) {
  const { user } = useAuth();
...
export function useAnalyticsTemplates(period: DashboardPeriod = '30d', enabled = true) {
  const { user } = useAuth();
