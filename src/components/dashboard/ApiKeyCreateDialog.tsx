...
import {
  ...
  type ApiKeyScope,
  defaultScopesForEnvironment,
  scopesForEnvironment,
...
interface ApiKeyCreateDialogProps {
  ...
  onCreated: (result: { plainKey: string; name: string; environment: ApiKeyEnvironment }) => void;
}
...
export function ApiKeyCreateDialog({
  ...
  const { create } = useApiKeyMutations(environment);
  const [name, setName] = useState('');
  const [scopes, setScopes] = useState<ApiKeyScope[]>(defaultScopesForEnvironment(environment));

  const scopeOptions = availableScopes ?? [...scopesForEnvironment(environment)];

  useEffect(() => {
    if (!open) {
      setName('');
      setScopes(defaultScopesForEnvironment(environment));
    }
  }, [open, environment]);
  ...
  const placeholder =
    environment === 'test' ? 'e.g. CI · staging' : 'e.g. Production · web-app';
        ...
        <form onSubmit={handleSubmit}>
          <DialogHeader>
            <DialogTitle className="text-base font-medium">
              Create {environment === 'test' ? 'test' : 'live'} API key
            </DialogTitle>
