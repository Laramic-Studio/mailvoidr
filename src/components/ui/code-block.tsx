...
.dark .cbhl .line-highlighted{background-color:oklch(0.769 0.188 70.08/.15);border-left-color:oklch(0.828 0.189 84.429)}
.shiki,.shiki span{background-color:transparent!important}
.dark .shiki,.dark .shiki span{color:var(--shiki-dark)!important;background-color:transparent!important;font-style:var(--shiki-dark-font-style)!important;font-weight:var(--shiki-dark-font-weight)!important;text-decoration:var(--shiki-dark-text-decoration)!important}
`;

function injectStyles() {
  if (typeof document === "undefined") return;
  const existing = document.getElementById("ss-code-block");
  if (existing) {
    existing.textContent = CB_STYLES;
    return;
  }
  ...
  return (
    <div className={cn("rounded-lg border overflow-hidden", className)}>
      <div className="flex items-center justify-between border-b bg-muted/50">
        ...
        {file ? <div className="pr-3 shrink-0"><CopyBtn code={file.code} /></div> : null}
      </div>
      {file ? (
        <CodeRenderer
          code={file.code}
